...
<<<<<<< HEAD
        ...
        if (QRAPI.getQRResult(qrresult))
        {
            for (int i = 0; i < TotalNumber; i++)
            {
                ...
                if (qrresult[i, 0] != 99 && qrresult[i, 0] >= 0)
                {
                    ...
                    Debug.Log(" "+newdata[i][1] + " " + newdata[i][2] + " " + newdata[i][3] + " " + newdata[i][4]);
                }
            }
        }
    ...
    public void Location_Updater(float[][] Current, int[][]QRinfo) { // put data into CurrentLocation[][], repackage data
        ...
        for (int i = 0; i < TotalNumber; i++)
        {
            ...
            if (QRinfo[i][0] != 99)
            {
                ...
                Current[QRinfo[i][0]][3] = QRinfo[i][4]- TemCurrentDegree;

                TemCurrentDegree = QRinfo[i][4];
    ...
    void change_position(float[] Current, GameObject Objects) // 
    {
       ...
       // if (Objects.transform.rotation.y != Current[3]/180)
           // Objects.transform.Rotate(0, (Current[3]/180  - Objects.transform.rotation.y), 0, Space.World);

        //Objects.transform.Rotate(0, ((Current[3]-180)/180-Objects.transform.rotation.y), 0, Space.World);
        // Debug.Log("Current[3]:" + Current[3] + "Current3/180=" +( Current[3] / 180 )+ "y=" + Objects.transform.rotation.y );
